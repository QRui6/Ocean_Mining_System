...
export async function loadWindMeta() {
    try {
        console.log('📋 开始加载风场元数据（从API）...');
        const metadata = await fetchWeatherMetadata('wind');
        ...
        console.log('✅ 风场元数据加载成功（API）:', meta);
        return meta;
    } catch (error) {
        console.error('❌ 加载风场元数据失败（API）:', error);
        throw error;
    }
}
...
export async function loadGlobalWindData(timeIndex = 0) {
    try {
        ...
        // 3. 从API加载数据（二进制格式）
        console.log('📦 使用二进制传输加载风场数据...');
        const arrayBuffer = await fetchWeatherDataBinary('wind', timeIndex);
        
        // 4. 解析二进制数据
        const windData = parseBinaryWeatherData(arrayBuffer, meta);
        
        // 5. 数据统计
        console.log('📈 风场数据统计:', {
            U分量范围: `${windData.u.min.toFixed(2)} ~ ${windData.u.max.toFixed(2)} m/s`,
            V分量范围: `${windData.v.min.toFixed(2)} ~ ${windData.v.max.toFixed(2)} m/s`,
            数据点数: windData.u.array.length
        });
        
        console.log('✅ 风场数据加载完成（二进制传输）');
        ...
        console.error('❌ 加载风场数据失败（API）:', error);
        throw error;
    }
}

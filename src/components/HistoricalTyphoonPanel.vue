        ...
        >
            ...
            >
                ...
                <div class="mb-4 flex items-center border-b-2 border-cyan-500/40 pb-3">
                    ...
                    <h3 class="text-2xl font-bold tracking-wider text-white">历史台风查询</h3>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="border-t border-dashed border-slate-700/60 pt-3">
                        <div class="mb-3 flex items-center justify-between">
                            ...
                            <span class="text-xs text-slate-400">以区域边界为准</span>
                        </div>
                        <div class="grid grid-cols-3 gap-2">
                            <button
                                v-for="buffer in [100, 300, 500]"
                                :key="buffer"
                                type="button"
                                :class="['query-btn', Number(filters.bufferKm) === buffer ? 'query-btn-active' : '']"
                                @click="updateFilters({ bufferKm: buffer, page: 1 })"
                            >
                                <span>{{ buffer }} km</span>
                            </button>
                        </div>
                    ...
                    <div class="current-area-strip">
                        ...
                        <em>{{ selectedAreaRepresentative }}</em>
                    </div>
            ...
            >
                ...
                <div class="mb-3 flex shrink-0 items-center justify-between">
                    <div class="section-title">历史台风列表</div>
                    <span class="text-xs text-slate-400">共 {{ totalEvents }} 条</span>
                </div>

                <div class="mb-3 shrink-0 space-y-2 border-y border-dashed border-slate-700/60 py-3">
                    <div class="grid grid-cols-4 gap-2">
                        <button
                            v-for="level in impactLevels"
                            :key="level.value"
                            type="button"
                            :class="['filter-btn', filters.impactLevel === level.value ? 'filter-btn-active' : '']"
                            @click="updateFilters({ impactLevel: level.value, page: 1 })"
                        >
                            {{ level.label }}
                        </button>
                    </div>
                    <div class="grid grid-cols-3 gap-2">
                        <button
                            ...
                            :key="option.value"
                            type="button"
                            :class="['filter-btn', filters.sort === option.value ? 'filter-btn-active' : '']"
                            @click="updateFilters({ sort: option.value, page: 1 })"
                        >
                            {{ option.label }}
                        </button>
                    </div>
                ...
                <div class="custom-scrollbar min-h-0 flex-1 space-y-2 overflow-y-auto pr-2">
                    <div v-if="loading" class="empty-box">正在加载历史台风列表...</div>
                    <div v-else-if="errorMessage" class="empty-box">历史台风列表暂不可用</div>
                    <div v-else-if="!events.length" class="empty-box">当前区域暂无历史台风记录</div>
                    <template v-else>
                        <button
                            v-for="event in events"
                            :key="event.sid"
                            type="button"
                            :class="['event-card', selectedEvent?.sid === event.sid ? 'event-card-active' : '']"
                            @click="emit('selectEvent', event)"
                        >
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0">
                                    <div class="truncate text-base font-bold text-white">{{ event.name || event.sid }}</div>
                                    <div class="mt-1 truncate text-xs text-slate-400">{{ event.season || '--' }} · {{ event.basin || '--' }}</div>
                                </div>
                                <span :class="['impact-badge', getImpactClass(event.impactLevel)]">
                                    {{ getImpactLabel(event.impactLevel) }}
                                </span>
                            </div>
                            <div class="event-metric-grid">
                                <div><span>最近距离</span><strong>{{ formatMetric(event.minDistanceKm, 0) }} km</strong></div>
                                <div><span>最大风速</span><strong>{{ formatMetric(event.maxWindNearArea, 0) }} kt</strong></div>
                                <div><span>影响时长</span><strong>{{ formatDuration(event.influenceDurationHours) }}</strong></div>
                            </div>
                            <div class="mt-2 border-t border-slate-700/60 pt-2 text-xs text-slate-400">
                                最近时间 {{ formatTime(event.closestTime) }}
                            </div>
                        </button>
                    </template>
                </div>
...
const emit = defineEmits(['selectArea', 'selectEvent', 'pageChange', 'filtersChange']);

const impactLevels = [
    { value: '', label: '全部' },
    { value: 'core', label: '核心' },
    { value: 'strong', label: '强影响' },
    { value: 'outer', label: '外围' }
];

const sortOptions = [
    { value: 'min_distance', label: '距离最近' },
    { value: 'max_wind', label: '风速最高' },
    { value: 'season_desc', label: '时间最新' }
];
...
const selectedAreaRepresentative = computed(() => (
    props.selectedArea ? '整个区域统计' : '--'
));

const totalPages = computed(() => (
    Math.max(1, Math.ceil((Number(props.totalEvents) || 0) / (Number(props.pageSize) || 1)))
));
...
const isSelectedArea = (area) => {
    const currentKey = String(props.selectedArea?.areaKey || props.selectedArea?.id || '');
    const areaKey = String(area?.areaKey || area?.id || '');
...
const formatDuration = (hours) => {
    ...
    return numericHours < 24 ? `${numericHours.toFixed(0)} h` : `${(numericHours / 24).toFixed(1)} 天`;
};
...
const getImpactClass = (value) => ({
    'impact-core': value === 'core',
    'impact-strong': value === 'strong',
    'impact-outer': value === 'outer'
});
</script>

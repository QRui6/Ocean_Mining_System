            ...
            <div class="flex-1 overflow-y-auto overflow-x-auto custom-scrollbar px-4 py-2 min-h-0">
                <table class="w-full text-left border-collapse">
                    <thead class="bg-blue-900/20 text-blue-200 text-sm sticky top-0 backdrop-blur-md z-10">
                        <tr>
                            <th v-for="h in ['序号', '航线名称', '长度(km)', '途经区域', '附近港口', '流量等级']" :key="h" 
                    ...
                    <tbody class="text-slate-300 text-sm font-['Rajdhani']">
                        ...
                        >
                            ...
                            <td class="px-3 py-2 font-mono text-yellow-400/90 font-bold text-xs">{{ formatDistance(item.length) }}</td>
                            <td class="px-3 py-2 opacity-80 text-xs font-['Noto_Sans_SC']">{{ item.region }}</td>
                            <td class="px-3 py-2 opacity-80 text-xs font-['Noto_Sans_SC']">{{ item.nearbyPortsCount }} 个</td>
                            <td class="px-3 py-2 font-['Noto_Sans_SC']">
                                <span :class="getTrafficLevelClass(item.trafficLevel)">
                                    {{ item.trafficLevel }}
                                </span>
                            </td>
                        </tr>
...
export default {
    ...
    setup(props, { emit }) {
        ...
        const getTrafficLevelClass = (level) => {
            if (level === '高') return 'text-green-400 text-xs font-bold';
            if (level === '中') return 'text-yellow-400 text-xs';
            if (level === '低') return 'text-orange-400 text-xs';
